...
The `src/parser/` directory contains PocScript's syntax analyzer and AST structure. It receives the token list generated by the lexer and produces a tree that represents the structure of the program.

## Files

- `include/parser.h` and `src/parser/parser.c`: recursive descent parser and the `parserParseProgram` entry point
- `include/ast.h` and `src/parser/ast.c`: node types, utility functions, textual serialization, and memory cleanup
...
The node types defined in `include/ast.h` cover four main groups.
...
- `AST_FOR`: `for` loop
- `AST_RETURN`: `ret`, with or without a value
- `AST_BREAK`: `break`
